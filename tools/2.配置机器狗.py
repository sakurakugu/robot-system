...
def main():
    ...
    try:
        ...
        while True:
            ...
            print("2. 安装软件 (手机端可用)") # TODO: 让手机端安装软件可以先找本体自带的包，然后在找服务器下载的包，然后在安装最新版的，之后就把这个脚本中的 6.打包功能移动到常用命令中，然后把这个脚本可以删除了。使命结束了
            print("3. 群控配置 (手机端/本体可用)")
            print("4. SSH 登录 (手机端可用)")
            print("5. 扫描设备 (手机端可用)")
