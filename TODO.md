...
1. 群控、编舞，然后是ai优化、物品识别并走过去（能用但都一般般），然后人脸识别还没搞

2. 向后走 步数 是 -2 并没有向后走，而是向前走

3. 改一下动作的提示词

4. https://grpc.org.cn/docs/what-is-grpc/introduction/
   让机器人端通过 grpc 连接到后端的 python 服务，然后把结果返回给 nodejs 服务 （感觉还是直接将edge音频的改成阿里的，然后直接改成nodejs更好）

5. 让大模型调用工具而不是通过提示词来行动
...
6. 关闭 sdk 模式后，可以尝试让手机端通过智元自带的来控制移动

7. 所有端的操作页面的是否启用了sdk改成查询机器狗本地是否真的开启了，而不是用默认值

8. 实现 `last_action_before_disable` 功能
...
10. 记得把服务器 8554 端口开出来，否则前端无法连接到后端的视频推流

11. 现在云端的电量一直在0%和49%一直跳
