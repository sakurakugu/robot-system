...
> 这是旧的通过笔记本进行群控跳舞才需要安装环境，新的代码已经进行优化了，不需要只能用Linux系统了
...
1.  **安装编译Python所需的依赖**
    ...
    tk-dev libffi-dev wget curl git python-is-python3
    ```
...
> 需要win11的24h2以上才可以，win10等其他的不支持镜像模式
