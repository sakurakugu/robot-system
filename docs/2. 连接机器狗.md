...
```bash
# 如果要连接WiFi就先通过AP/有线连接上机器狗，然后打开脚本，选择5），然后打开GUI页面进行连接
...
3. **本机连上 WIFI 后，可以使用机械狗局域网 IP 进行 ssh 远程登录**
   ...
   > 很重要的一点，如果电脑所连接的WIFI是2.4G和5G合并的网络，建议将网络分开，或者直接将电脑通过有线网连接路由器
   > 还有，如果电脑也不建议同时连接WIFI和有线，除非保证连接有线的网络和机械狗在同一局域网内，否则同理
   > 坑了我一天半，包括但不限于修改代码、固定路由、重置网络、重置机器人、重装电脑，最后发现是这个原因
