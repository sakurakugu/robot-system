...
1. 在手机端的 `设置` -> `无线和网络` -> `WIFI` 中连接机器狗的 WIFI 网络。
2. 然后打开机器狗手机端的 `发现` -> `安装机器狗软件` -> `WIFI配置` 输入WiFi名称和密码，然后连接

#### 2. 使用脚本连接

```bash
# 启动常用命令脚本
python3 ./tools/2.常用命令.py
```

> 现在脚本里只保留了“安装软件”和“打包 robot-agent 套件”，WIFI、群控等配置改为在手机端操作。

1. 通过AP/有线网络连接机器狗（密码一般在🎮后面（agibot001））
2. 点击 `6.`，先设置安装目标机器狗，再安装 `sparkrobot-common` 和 `robot-server`
3. 通过 http://机器狗IP:8080 访问图形页面
   (AP: 192.168.234.1, 有线:192.168.168.168)
4. 退出脚本
...
> 如果曾经安装过 robot-server，可以直接通过 http://机器狗IP:8080 访问
...
2. **机械狗连接 WIFI**
   ...
   > 成功连接 WIFI 后，重新通过wifi远程登录，
   >
   > 1. 使用上图获取的机械狗在wifi中的 IP（AP连接下）
   > 2. 或者通过 下载advanced ip scanner等工具扫描局域网 IP（通过mac地址查询））
   > 3. 也可以通过运行脚本 `5. 扫描设备` 来获取
